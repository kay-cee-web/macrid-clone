...
export function AnalyticsSection({ title, icon, stats, error, onRetry, children }: AnalyticsSectionProps) {
  return (
    <section className="grid gap-3">
      <h2 className="flex items-center gap-2 text-lg font-semibold [&_svg]:size-4 [&_svg]:text-muted">
        {icon}
        {title}
      </h2>
      {error && !stats ? (
        <div role="alert" className="flex flex-wrap items-center gap-3 rounded-[12px] bg-bad-soft px-4 py-3 text-sm text-bad">
