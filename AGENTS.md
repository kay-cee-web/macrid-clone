...
- **Plugins** (`/agents/[id]/plugins?tab=connectors|skills`):
  ...
  - **`GET /connectors` (seen 2026-09-24)** returns `{status, categories, connectors: [{key, name, category, desc, auth, store, fields, connected, status: "active"|"not_connected", account_email | key_hint, needs_action}]}`. `needs_action` on a connected row → Needs attention. Its field lists are the source for our email-platform forms (ConvertKit needs `api_secret`, ActiveCampaign `api_url`; list/group/form IDs are optional). It also lists `anthropic`, `openai`, `gemini` (category `ai`, handled by Models) and a legacy `sendinblue`, which the catalogue skips.
