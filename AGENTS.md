...
- **Workspace setup** (`src/lib/setup/`): `setupFrom(connections)` gives each platform one of `ready | shared | missing | unknown`.
  ...
  - whatsapp is `unknown` unless `/connectors` reports it connected: no endpoint reliably does, and `unknown` never blocks. facebook is an ordinary OAuth connector now (see Plugins), so it is `missing` until connected.
...
- **Plugins** (`/agents/[id]/plugins?tab=connectors|skills`):
  - **Layout and copy (backend spec, 2026-09-24):** groups are by maker or purpose — Google, Microsoft, Mailbox, Messaging, Payments, Sending, Prospect sources, Email platforms, Coming soon (`CONNECTOR_CATEGORIES`). **Every description says what the user gets, not which scope it needs, in under ~60 characters.**
    - **Three badges only** (`ConnectorCard`): Not connected, Connected, **Needs attention**. `ConnectionState.status` has `attention` for a connection that exists but broke — a Google service reporting an `error`, saved mailboxes none of which are active, a payment account with a `problem` or a webhook secret never set. `detail` then holds the problem, shown on the card; the button becomes Reconnect (Fix mailbox, Alerts for payments). `SetupState` carries it too, and `missingIn` counts it as missing.
    - **Messaging** (`data/connectors/messaging.ts`, `auth: "channel"`) is the open agent's WhatsApp and Telegram chat link, not a workspace connection: `ConnectorsPanel` reads it with `useAgentChannels` and merges it into the state, and the button links to `/agents/{id}/settings?section=channels&channel=…`, where the pairing flow lives. No platform points at these, so they never appear on idea cards.
  - **Payments** (`data/connectors/payments.ts`, `store: "payments"`, `src/services/payments.ts`; backend doc PAYMENTS.md): the user's own Stripe, PayPal, Paystack, Flutterwave, Paddle and Lemon Squeezy, read-only. Routes answer 401 without a token (probed 2026-09-24). **`GET /payments/providers` is confirmed** (2026-09-24): `{status, providers: [{provider, label, fields: [{name, label}], help}]}` — `provider` is already our connector key (`lemonsqueezy`), fields carry no type or per-field help (secret-looking names become password inputs; `help` is the dialog's description). A rejected key is a **422 `{status: false, message: "Stripe 401: Invalid API Key provided: sk_test_***0000"}`** with no `errors`; `PaymentConnectModal` shows the backend's message word for word in the form and toasts it too (the native `<dialog>` sits in the top layer and can cover a toast). `GET /payments/connections` is confirmed too: `{status, connections: [{id, provider, name, live, currency, is_active, last_synced, last_error}]}` — `name` is the provider's own, so the card shows `currency`; `last_error` or `is_active: false` means Needs attention. It says nothing about the webhook secret, so `fetchPaymentConnectionsWithWebhooks` reads each one's **`GET …/{id}/webhook`** as well — confirmed: `{status, webhook: {url, secret_set, verified, last_event, steps: [..]}}` (nested under `webhook`). `secret_set: false` (not Paystack) → Needs attention; `verified` / `last_event` show in the Alerts dialog. Normalisers live in `lib/payments/normalize.ts`, types in `types/payment.ts`. `POST …/notifications` answers `{status, message: "Alerts updated.", alerts: {events, channels, to, min_amount, verified}}` (seen 2026-09-24). **Neither the connections list nor the webhook route returns the saved alerts**, so the dialog reopens on the defaults and saving again would blank a stored number; `toNotify` reads an `alerts` block (or flat `notify_*`) wherever one appears, so it fills itself once the backend adds it.
    - `GET /payments/providers` builds the key form (`PaymentConnectModal`, sharing `CredentialFields` with `ApiKeyModal`); the connector's own `fields` are the fallback. `POST /payments/connections {provider, credentials, live}` — the backend tests the key first and never returns it. `live` is a switch for Paddle and PayPal only (`HAS_SANDBOX`). Provider keys are matched through `paymentKey` ("lemon_squeezy" = "lemonsqueezy").
    - A connected card offers **Alerts / Test / Disconnect** (`PaymentActions`). Alerts (`PaymentAlertsModal`) shows the connection's webhook URL and steps (`GET …/{id}/webhook`, `WebhookLink`), the signing secret (not Paystack, which signs with its key — `SIGNS_WITH_KEY`), events (`PAYMENT_EVENTS`), a WhatsApp number and a minimum amount → `POST …/{id}/notifications`. Secrets are never read back, so a blank field keeps the saved one. WhatsApp alerts need Twilio's `payment_alert` template, until then only within 24h of the user messaging the number — the dialog says so.
    - The `payments` platform (any of the six; Stripe stands for them on cards) makes the Money handling cards real: New payment alert, Revenue this month, Did they pay?, Refunds and disputes. Invoice chasing only gets halfway — nothing stores invoices.
  - **Email platforms** (`data/connectors/emailPlatforms.ts`, `store: "email_platforms"`, `src/services/emailPlatforms.ts`; backend doc 2026-09-26): Mailchimp, Brevo, Klaviyo, ConvertKit, ActiveCampaign, MailerLite, GetResponse and Systeme.io, **contacts only in both directions** — `pull` imports their subscribers as leads, `push` sends leads out. **No campaign creation and no sending through them**, so no card description may imply it. All key-based, no OAuth, and live (probed 2026-09-26).
    - **Connecting is two steps.** `POST /email-platforms {platform, credentials}` tests the key, saves it encrypted (never returned) and **answers with the platform's lists**, so the target is chosen from real lists instead of typed in — hence no `list_id`/`form_id`/`group_id` field in any key form. `POST /email-platforms/{id}/list {list_id}` stores the choice; `GET /email-platforms/{id}/lists` re-reads them. `GET /email-platforms/providers` carries the fields, help text and each platform's own word for a list.
    - **`GET /email-platforms` answers `{status, connections: [...]}`** — `connections`, **not** `platforms` (confirmed with a real Mailchimp key, 2026-09-26). Row: `{id, platform: "mailchimp", name: "Mailchimp", list_word: "audience", list: null, list_id: null, pushed: 0, pulled: 0, is_active: true, last_synced: "2 seconds ago", last_error: null}`. `name` is the platform's own name, so the chosen list's name is `list`; `last_synced` is prose ("2 seconds ago"), not a timestamp.
    - **Say what their screen says** (`LIST_WORDS`): Mailchimp *audience*, ConvertKit *form*, MailerLite *group*, GetResponse *campaign*, Systeme.io *tag*, the rest *list*. The API sends it too; ours is the fallback. **Systeme.io has no lists** (`LISTLESS`) — it groups by tag, so push needs no target and it's exempt from the check below.
    - **A saved key with no list chosen is Needs attention** (`applyEmailPlatforms`): connected, but nothing can sync until a target is picked — the same call as a payment account with no webhook secret. `EmailPlatformActions` then offers **Choose/Change `<word>`** (never Reconnect — the key is fine) and Disconnect, opening `EmailPlatformListModal`. The modal reads `/email-platforms` for the connection id itself rather than trusting the card's `recordId`, which is stale right after a connect. **A fresh connect goes straight into it**, so the flow never ends on a card that says something's wrong.
    - **Syncing is the agent's job, not a button.** `sync_email_platform` runs pull and push from chat ("import my Mailchimp audience"), so the UI's part is connecting and pointing it at a list. `syncEmailPlatform` exists in the service for when that changes.
    - **The unsubscribe rule is the backend's and isn't configurable:** pull drops unsubscribed contacts, push sends only leads with `status = 1`. Never offer to include them. `platformKey` maps the backend's spelling to our key ("systeme_io" → `systeme`), the way `paymentKey` does.
  - **Social** (`data/connectors/social.ts`, `store: "social"`, `src/services/social.ts`; backend doc 2026-09-26): Facebook Page, Instagram, LinkedIn, X, YouTube and TikTok. An agent writes the post, makes the image with `generate_image`, publishes it and reads back how it did; the ad accounts ride along on the same grant and are read-only.
    - **Reads are live (confirmed 2026-09-26).** `GET /social/platforms` → `{status, platforms: [{platform: "facebook", label: "Facebook Page", kind: "social"|"ads", can: ["text","image",…], needs: "…", connected: []}]}` — **ten rows**, six `social` and four `ads` (`meta_ads`, `tiktok_ads`, `linkedin_ads`, `google_ads`). Note the field names: capabilities are `can`, the review requirement is `needs`, accounts are `connected`. `GET /social/accounts` → `{status, accounts: []}`. **`GET /social/posts` is a Laravel paginator**, so rows sit at `posts.data`.
      - Earlier this day the same routes answered 500 (`Target class [DexiSocialController] does not exist`) and before that 404. **Without a token they answer 401 regardless**, because auth middleware runs before the controller resolves — so a 401 proves a route is *registered*, never that it works. Always probe with a token.
    - **Consent is one route (backend contract, 2026-09-26, not yet deployed):** `GET /social/redirect?platform=<meta|linkedin|x|tiktok|youtube>` → `{status, auth_url}`, and `GET /social/callback/{platform}` closes the popup and posts back the way the Google connector does. Identical shape to `/connectors/google/redirect`, so `fetchOAuthUrl` and `useOAuthPopup` need no social-specific code.
      - **The query parameter is the grant, not the platform — five flows for six cards.** Meta's one grant covers the Page, the linked Instagram account and Meta Ads, so `facebook_page` and `instagram` both send `platform=meta` and connecting either lights up both. YouTube and Google Ads go through the existing Google client.
      - The consent flow itself stays the backend's: the token exchange needs a client secret that can never ship in frontend code, and there's no `POST /social/accounts` to store a grant. We start the flow; we never complete it.
      - Side effect of not being `planned`: LinkedIn, X, Instagram and TikTok now count as `missing` rather than `planned` in `setupFrom`, so the handful of **prospecting, creative and research** ideas tagged with them read "Connect LinkedIn" instead of passing silently. The social *publishing* ideas are unaffected — they carry `idea.blocked`, which `readinessOf` checks first.
    - **The platform review rules are ops knowledge and stay out of the UI.** Each platform demands something before it will post to anyone else's account — Meta app review and Business Verification, LinkedIn's Community Management access, a paid X tier, Google's upload-scope verification, TikTok's Content Posting audit — roughly 2–6 weeks each, and Instagram additionally refuses personal accounts (Business or Creator, linked to a Page). That belongs in a message to whoever registers the developer apps, not on a card a customer reads. `SocialPlatform.review` carries it **only if the backend sends it**; we keep no fallback copy to drift out of date. **Your own accounts work in dev mode**, so the flow can be tested before review clears.
    - `facebook_page` is a separate connector from `facebook` (Prospect sources): different grant, different job. `socialKey` maps the backend's spelling onto ours.
    - The read group is wired (`applySocialAccounts`), so the 500 shows on the Plugins page as a named problem rather than being swallowed. Shapes in `lib/social/normalize.ts` are **guesses** — no payload has ever been seen — so confirm them against the first live response.
  - **`GET /connectors` (seen 2026-09-24)** returns `{status, categories, connectors: [{key, name, category, desc, auth, store, fields, connected, status: "active"|"not_connected", account_email | key_hint, needs_action}]}`. `needs_action` on a connected row → Needs attention. It also lists `anthropic`, `openai`, `gemini` (category `ai`, handled by Models) and a legacy `sendinblue`, which the catalogue skips. It **no longer speaks for the email platforms** — see the grouping rule below.
  - **Connectors:** catalogue in `src/data/connectors`; API in `src/services/connections.ts`; OAuth via `useOAuthPopup`; key forms built from each connector's field list in `ApiKeyModal`.
    - **Every family is read from its own route, and `/connectors` covers the rest.** The backend keeps each family in its own group, so `src/services/connectionSources.ts` declares one `GROUPS` entry per family (email platforms, payments, mailboxes, Google services, SMTP senders, SMS senders, the Places key), each pairing its route with an applier in `lib/connections/ownedState.ts`. `/connectors` — with `/integrations` behind it — is the leftover reader, "others": Outlook, Facebook, WhatsApp Business.
      - **Order is the mechanism:** `readOthers` first, then `readOwnedGroups` over the top. A group that answers wins, because its applier writes *every* one of its connectors including `disconnected` — so an old `/integrations` key row can't leave Mailchimp looking connected when `/email-platforms` has nothing. A group that **fails** leaves whatever `/connectors` said rather than blanking the section, and says why in `problems`.
      - **Adding a family** is one `GROUPS` entry, one applier and a `store` on its connectors — not another override bolted after the read. `store` also picks the write route in `connectApiKey`/`disconnectConnector`. Social was added exactly that way.
      - Appliers live in two files by what they own: `ownedState.ts` for our own mail and identity credentials (Google, mailboxes, SMTP and SMS senders, the Places key), `ownedAccounts.ts` for somebody else's accounts (payments, email platforms, social). Both share `writer`, which stamps `owner`.
      - A failed group names itself in `problems` (`Could not load your social accounts — …`), since two groups failing with the same "Server Error" would otherwise be two identical lines. `problems` shows on the Plugins page only, never on idea cards.
    ...
    - WhatsApp Business has no connect route of ours, so its action is "Connect" / "Manage in Dexisphere", a link to the main app's settings page, in the popover too. **That link 404s today** (`app.dexisphere.com` is this app); WhatsApp needs Meta's embedded signup plus a backend status route. **Facebook is OAuth now** (2026-09-24): `/connectors` lists it with `connect: "/connectors/facebook/redirect"`.
    - **"Others" is the backend's to-do list.** `src/data/connectors/planned.ts` holds connectors a workflow needs that have **no route of any kind yet** (`category` and `auth` both `"planned"`). They look like any other card — badge "Not connected" — and **Connect toasts "<name> is a work in progress."** and nothing more. It doesn't say *why*, because the reason would be our guess at the backend's state: a route we probed under a name we invented answering 404 is not the same as that route not existing. There is no "Coming soon" badge or section anywhere; a shelf isn't a status.
      - **The rule this follows: a card states what we know, never what we assume.** Integration status, review queues and backend defects go to the team that can act on them — a message, this file — not into UI a customer reads. When we become sure, we tell the right department, not the user.
      - **A platform whose routes exist but don't work yet does not belong here.** Build it properly with its real `auth` and routes so its errors surface and get fixed — that's why the six social connectors are `oauth` even though the backend 500s. Reserve `planned` for "nothing to call at all".
      - When a workflow needs a service that isn't in the catalogue, add it there rather than leaving the card without a logo. `workflowsNeeding` still counts what each one holds up.
  ...
  - **External connectors** (WhatsApp Business via Meta) link out to the Macrid app at `NEXT_PUBLIC_MACRID_APP_URL` (default `https://app.macrid.com`).
  ...
  - `ConfirmModal` is the shared "are you sure?" dialog, and `ui/Alert` is the error block a form puts **above its fields**: a native `<dialog>` sits in the top layer and can cover a toast, so anything toasted from inside one is shown there as well, never instead. `ApiKeyModal`'s `onConnected` means "saved" and leaves closing to the caller, which is how a connect can lead into the next dialog; `onClose` means dismissed.
